...
- **VS Code's `safeStorage` is a separate mechanism.** The encrypted secret FireConnect writes for VS Code lives in VS Code's own `state.vscdb` and is encrypted with Electron `safeStorage` (Chromium OSCrypt): on macOS, `v10` + AES-128-CBC keyed from the "Code Safe Storage" master password held in the login keychain (PBKDF2, 1003 iterations); on Windows, `v10` + AES-256-GCM with a 32-byte key DPAPI-protected (`CryptProtectData`, CurrentUser) and stored in VS Code's `Local State` file (`os_crypt.encrypted_key`); on Linux, `v11` + AES-128-CBC keyed from a keyring-held master password (PBKDF2, **1** iteration), or `v10` with the hardcoded "peanuts" password (1 iteration) as the "basic text" fallback. **Caveat:** on a Linux system with no keyring available, the "basic text" fallback is obfuscated, not meaningfully encrypted.
