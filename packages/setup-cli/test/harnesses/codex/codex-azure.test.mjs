...
describe("codex azure harness", () => {
  it("does not install the Fireworks shell hook for Codex or Azure mode", async () => {
    await withHome(async (home) => {
      await seedKeychainConfig(home, "fw_stored_but_unused_in_azure");
      ...
      for (const name of [".zshrc", ".bashrc", ".bash_profile"]) {
        ...
        if (existsSync(filePath)) {
          assert.doesNotMatch(await readFile(filePath, "utf8"), new RegExp(SHELL_HOOK_BEGIN));
        }
      }
  ...
  it("writes a Foundry provider table with wire_api chat and a literal bearer (--api-key)", async () => {
    await withHome(async (home) => {
      ...
      const result = await runFireconnect(
        ["codex", "on", "--azure", "--base-url", AZURE_ENDPOINT, "--api-key", AZURE_KEY, "--model", "FW-MiniMax-M2.5"],
        { HOME: home },
      );
      assert.equal(result.code, 0, result.stderr);
  ...
  it("normalizes a portal project endpoint to the resource-root base", async () => {
    await withHome(async (home) => {
      ...
      assert.match(toml, /model = "FW-GLM-5\.2"/);
    });
  });
  ...
  it("uses env_key AZURE_API_KEY when the key comes from the environment", async () => {
    await withHome(async (home) => {
      ...
      const status = await runFireconnect(["codex", "status"], { HOME: home });
  ...
  it("status reports the azure provider and endpoint", async () => {
    await withHome(async (home) => {
      ...
      const human = await runFireconnect(["codex", "status"], { HOME: home });
  ...
  it("re-on without --base-url reuses the stored endpoint and applies --model", async () => {
    await withHome(async (home) => {
      ...
      const reon = await runFireconnect(
        ["codex", "on", "--azure", "--model", "FW-MiniMax-M2.5"],
        { HOME: home },
      );
  ...
  it("switching from the Fireworks gateway to Azure does not inherit the gateway model", async () => {
    await withHome(async (home) => {
      ...
      const fwOn = await runFireconnect(
        ["codex", "on", "--api-key", "fw_test_key_12345", "--model", "glm-5p1"],
        { HOME: home, FIREWORKS_API_KEY: "" },
      );
      assert.equal(fwOn.code, 0, fwOn.stderr);

      // Switch to Azure WITHOUT --model: must fall back to the Foundry default,
  ...
  it("switching from Azure to the gateway does not reuse the Azure key as the Fireworks key", async () => {
    await withHome(async (home) => {
      ...
      assert.match(toml, /experimental_bearer_token = "fw_env_key_12345"/);
    });
  });
