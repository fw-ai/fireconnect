...
import { deepagentsConfigPath } from "../../../lib/harnesses/deepagents/core.mjs";
...
async function withHome(fn) {
  const home = await mkdtemp(path.join(os.tmpdir(), "fc-deepagents-azure-"));
  try {
...
describe("deepagents azure harness", () => {
  it("writes a fireworks-azure provider table with a literal api_key (--api-key)", async () => {
    await withHome(async (home) => {
      ...
      const result = await runFireconnect(
        ["deepagents", "on", "--azure", "--base-url", AZURE_ENDPOINT, "--api-key", AZURE_KEY, "--model", "FW-MiniMax-M2.5"],
        { HOME: home },
      );
      assert.equal(result.code, 0, result.stderr);

      const toml = await readFile(configPath, "utf8");
      assert.match(toml, /default = "fireworks-azure:FW-MiniMax-M2\.5"/);
      assert.match(toml, /\[models\.providers\.fireworks-azure\]/);
      assert.match(toml, new RegExp(`base_url = "${AZURE_BASE_URL.replace(/[.]/g, "\\.")}"`));
      assert.match(toml, /models = \["FW-MiniMax-M2\.5"\]/);
  ...
  it("normalizes a portal project endpoint to the resource-root base", async () => {
    await withHome(async (home) => {
      ...
      assert.match(toml, /default = "fireworks-azure:FW-GLM-5\.2"/);
    });
  });

  it("uses api_key_env AZURE_API_KEY when the key comes from the environment", async () => {
    await withHome(async (home) => {
      const configPath = deepagentsConfigPath(home);
      const result = await runFireconnect(
        ["deepagents", "on", "--azure", "--base-url", AZURE_ENDPOINT],
        { HOME: home, AZURE_API_KEY: AZURE_KEY },
      );
      assert.equal(result.code, 0, result.stderr);
      const toml = await readFile(configPath, "utf8");
      assert.match(toml, /api_key_env = "AZURE_API_KEY"/);
      assert.doesNotMatch(toml, new RegExp(AZURE_KEY));
    });
  });
  ...
  it("fails without a base URL", async () => {
    await withHome(async (home) => {
      const result = await runFireconnect(
        ["deepagents", "on", "--azure", "--api-key", AZURE_KEY],
        { HOME: home },
      );
  ...
  it("on/off round-trip restores the original config byte-for-byte", async () => {
    await withHome(async (home) => {
      ...
      const on = await runFireconnect(
        ["deepagents", "on", "--azure", "--base-url", AZURE_ENDPOINT, "--api-key", AZURE_KEY],
        { HOME: home },
      );
  ...
  it("status reports the azure provider and endpoint", async () => {
    await withHome(async (home) => {
      ...
      const status = await runFireconnect(["deepagents", "status", "--json"], { HOME: home });
      ...
      const human = await runFireconnect(["deepagents", "status"], { HOME: home });
  ...
  it("re-on without --base-url reuses the stored endpoint and applies --model", async () => {
    await withHome(async (home) => {
      ...
      const reon = await runFireconnect(
        ["deepagents", "on", "--azure", "--model", "FW-MiniMax-M2.5"],
        { HOME: home },
      );
  ...
  it("switching from the Fireworks gateway to Azure does not inherit the gateway model", async () => {
    await withHome(async (home) => {
      ...
      const fwOn = await runFireconnect(
        ["deepagents", "on", "--api-key", "fw_test_key_12345", "--model", "glm-5p1"],
  ...
  it("switching from Azure back to the gateway replaces the provider and drops the azure key", async () => {
    await withHome(async (home) => {
      const configPath = deepagentsConfigPath(home);
      await runFireconnect(
        ["deepagents", "on", "--azure", "--base-url", AZURE_ENDPOINT, "--api-key", "az-secret-xyz-999"],
        { HOME: home },
      );
      const fwOn = await runFireconnect(
        ["deepagents", "on"],
      ...
      const toml = await readFile(configPath, "utf8");
      assert.doesNotMatch(toml, /\[models\.providers\.fireworks-azure\]/);
