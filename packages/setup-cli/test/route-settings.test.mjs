...
import { FIREWORKS_BASE_URL, FIREWORKS_ENV_KEYS } from "../lib/fireconnect-core.mjs";
...
test("buildChallengerSettings: routes to Fireworks direct with the challenger model + inline key", async () => {
  ...
  // Model mapping pins ANTHROPIC_MODEL to the challenger router id. The [1m]
  // beta tag is appended by claudeCodeModelId — matches real `fireconnect claude on`.
  assert.equal(
    settings.env.ANTHROPIC_MODEL,
    "accounts/fireworks/routers/glm-5p2-fast[1m]",
  );
});
...
test("buildChallengerSettings: top-level model is the challenger (direct mode)", async () => {
  ...
  assert.equal(settings.model, "accounts/fireworks/routers/glm-5p2-fast[1m]");
});
