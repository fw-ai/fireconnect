...
} from "../lib/key-storage-cache.mjs";
import { fileDataRoot } from "../lib/builtin-file-secret-store.mjs";
import { plaintextSecretPath, readPlaintextSecret, writePlaintextSecret } from "../lib/plaintext-secret-store.mjs";
...
describe("plaintext secret fallback", () => {
  ...
  it("getSecret falls through stale plaintext cache to secure storage", async () => {
    ...
    const { writeKeyStorageCache } = await import("../lib/key-storage-cache.mjs");
