...
import { link, openInBrowser } from "./term.mjs";

/**
 * Browser sign-in for `fireconnect login` (login_roadmap.md Stage 2).
 *
...
.mark path{fill:${ok ? "#6720FF" : "#3a3546"}}
 ...
 * error/timeout. `openUrl` runs after the port is bound (the listener must
 * exist before the browser navigates); when it resolves false the wait is
 * abandoned so the caller can fall through to the next flow instead of
 * sitting out the timeout. Binding failures resolve the same way.
 * @param {string} authorizeUrl
 * @param {string} state
 * @param {(url: string) => Promise<boolean>} openUrl
 * @param {{ timeoutMs?: number, port?: number }} [options]
 * @returns {Promise<{ code: string, failure: string, fatal: boolean }>}
 */
function waitForCallback(authorizeUrl, state, openUrl, { timeoutMs = CALLBACK_TIMEOUT_MS, port = CALLBACK_PORT } = {}) {
  return new Promise((resolve) => {
    ...
    let settled = false;
    const finish = (result) => {
      // First outcome wins: the timeout, a callback in flight when it fired,
      // and an openUrl failure can all race here.
      if (settled) {
        return;
      }
      settled = true;
    ...
    server.listen(port, "127.0.0.1", async () => {
      ...
      if (!(await openUrl(authorizeUrl))) {
        finish({ code: "", failure: "Couldn't open a browser on this machine.", fatal: false });
      }
    });
  });
}
 ...
 * @param {{ onStatus?: (line: string) => void, openBrowser?: boolean, cognitoDomain?: string, clientId?: string, timeoutMs?: number }} [options]
...
export async function signInViaLocalhostCallback({
  ...
  const { code, failure, fatal } = await waitForCallback(authorizeUrl, state, async (url) => {
    ...
    return false; // abandon the wait; the caller cascades to the device flow
  }, { timeoutMs });
  if (!code) {
