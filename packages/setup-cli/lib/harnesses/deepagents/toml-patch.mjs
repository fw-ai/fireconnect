...
const FIREWORKS_AZURE_PROVIDER_TABLE_HEADER = "[models.providers.fireworks-azure]";

const MODELS_DEFAULT_LINE = /^default\s*=.+$/;
const MODELS_KV_LINE = /^([A-Za-z0-9_.-]+)\s*=/;
const DEEPAGENTS_API_KEY_ENV_NAME = "FIREWORKS_API_KEY";
const AZURE_API_KEY_ENV_NAME = "AZURE_API_KEY";

/**
 * @param {string} raw
 * @param {(sections: import("../../io/toml-section-utils.mjs").TomlSection[]) => void} mutate
 */
...
/**
 * @param {import("../../io/toml-section-utils.mjs").TomlSection} section
 ...
 *   modelId: string,
 *   baseUrl: string,
 *   authMode: import("./auth.mjs").DeepagentsAuthMode,
 *   apiKey?: string,
 * }} patch
 */
export function patchFireconnectRoutingRaw(raw, patch) {
  ...
  if (patch.authMode === "env-reference") {
    ...
    providerLines.push(`api_key = "${patch.apiKey}"`);
  }
 ...
 * `api_key` (when passed via --api-key) or the `api_key_env = "AZURE_API_KEY"`
 * reference. Foundry deployment names are used verbatim (no accounts/ prefix).
 *
 * @param {string} raw
 * @param {{
 *   modelId: string,
...
export function patchFireconnectAzureRoutingRaw(raw, patch) {
  const stripped = stripFireconnectRoutingRaw(raw, { stripModelsDefault: true });
  const modelSpec = `fireworks-azure:${patch.modelId}`;
  const withDefault = upsertModelsDefaultRaw(stripped, modelSpec);
  ...
  const providerLines = [
    ...
    patch.literalAuth
      ...
      : `api_key_env = "${AZURE_API_KEY_ENV_NAME}"`,
  ];

  const prefix = withDefault.trimEnd();
  return ensureTrailingNewline(`${prefix}\n${[
    FIREWORKS_AZURE_PROVIDER_TABLE_HEADER,
    ...providerLines,
    "",
  ].join("\n")}`);
}
 ...
 * @param {string} raw
 * @param {string} modelId
 * @param {import("./auth.mjs").DeepagentsAuthMode} [authMode="literal"]
 */
